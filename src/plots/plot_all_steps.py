...
def contour_from_threshold(
    ...
    """
    Retorna segmentos do contorno da máscara 
    temp <= threshold.
    """
    temp = np.asarray(temp)
    lon = np.asarray(lon, float)
...
def plot_whole_map_with_steps(
        dn, num = 44, threshold = -40):
...
fig = plot_whole_map_with_steps(
    dn, num = 11, threshold = -40)
