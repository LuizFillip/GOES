...
def find_nucleos(
    ...
    return_coords: bool = True,
    connectivity: int = 1,
    stats: bool = True,          # <-- novo
) -> pd.DataFrame:
    """
    Detecta regiões conectadas onde data <= temp_threshold e (opcionalmente)
    calcula estatísticas de temperatura por núcleo (min e mean).

    Retorna bounding box + área (graus^2 ou px) + stats (temp_min/temp_mean/n_pixels).
    """
    ...
    # máscara de núcleo (frio e finito)
    cold = (data <= temp_threshold) & np.isfinite(data)
    ...
    for i, sl in enumerate(objs, start=1):
        ...
        y_sl, x_sl = sl
        y0, y1 = y_sl.start, y_sl.stop
        x0, x1 = x_sl.start, x_sl.stop

        # máscara do rótulo i dentro do bounding box
        sub_lab = lab[y0:y1, x0:x1]
        mask_i = (sub_lab == i)

        # pixels reais do núcleo
        sub_data = data[y0:y1, x0:x1]
        vals = sub_data[mask_i]

        if vals.size == 0:
            continue

        # bounding box em coords ou índices
        if return_coords:
            lon0, lon1 = lons[x0], lons[x1 - 1]
            lat0, lat1 = lats[y0], lats[y1 - 1]
            ...
            area = (lon_max - lon_min) * (lat_max - lat_min)  # graus^2 (aprox)
            if area < area_threshold:
                continue

            row = [lon_min, lon_max, lat_min, lat_max, area]
        else:
            area_px = (x1 - x0) * (y1 - y0)
    ...
    if stats:
        columns += ["temp_min", "temp_mean", "n_pixels"]
